...
bot.on("message", async (msg) => {
  ...
  if (step === 2) {
    ...
    // await bot.sendMessage(chatId,'Ingrese una contraseña valida')
    if (count < paramsNum) {
      ...
      // contraseña = ""
      // confirmacion =""
/*       bot.sendMessage(chatId, '¿Desea firmar el documento?')
      bot.once('message', async(responseMsg)=>{
        let confirmacion = responseMsg.text.toUpperCase();
        if(confirmacion == 'SI'){
          await bot.sendMessage(chatId,'Ingrese una contraseña valida o proporcione una firma')
          bot.once('message', async (responseMsg) => {
            let contraseña = responseMsg.text;
            try {
              //Pasar los mensajes al script de python para que cree un nuevo documento con ellos
              firmaUsuario = await dbConfig.getRutaFirma(contraseña)
          ...
          // contraseña2.push(msg.text)
          // if(!contraseña2[2]){console.log(contraseña2)}
          //console.log(contraseña2)
          //contraseña2 = []
        }
      }) */
/*       contraseña2.push(msg.text.toUpperCase())
      console.log(contraseña2) */
    }
  ...
  /*   else if (step === 3) {
      ...
      for (const obj of csvContent) {
        ...
        // Llamar al método pythonInput con los valores como parámetros
        try {
          //Pasar los mensajes al script de python para que cree un nuevo documento con ellos
          pySpawner.pythonInput(values, documentData[0], documentData[1])
        } catch (err) {
          console.log('Error al esperar resultados')
        }
      }
    ...
    switch (data) {
      ...
      default: params = await getDocumentData(data)
        ...
        bot.once('message', async(responseMsg) => {
            ...
            if (confirmacion == 'SI') {
                await bot.sendMessage(chatId, 'Ingrese una contraseña válida o proporcione una firma');
                bot.once('photo', async (responseMsg2) => {
                  ...
                  // if(responseMsg.document.mime_type === 'image/png' || responseMsg.document.mime_type === 'image/jpg'){
                  await bot.sendMessage(chatId, 'Por favor ingrese una contraseña para poder acceder a la firma cuando se necesite')
                  bot.once('message', async (msg)=>{
                    ...
                    res = await dbConfig.newFirma(contraseña,filePath + file_name)
                    console.log(res)
...
/*             const handleFile = async (fileMsg) => {
              let photos = fileMsg.photo
              let highphoto = photos[photos.length-1]
              let docId = highphoto.file_id 
              ...
              bot.once('message', async (msg) => {
                  ...
                  const filePath = './../firmas/';
                  const filePath2 = await bot.downloadFile(docId, filePath);
                  let fileName = path.basename(filePath2);
                  const res = await dbConfig.newFirma(contraseña, filePath + fileName);
                  console.log(res);
                  bot.sendMessage(chatId, 'Debe ingresar por mensajes separados los siguientes parámetros en el siguiente orden: ' + params[0], { reply_markup: JSON.stringify(replyMarkup) });
                  step = 2;
              });
            };
            ...
            bot.once('message', async (responseMsg3) => {
              ...
              try {
                firmaUsuario = await dbConfig.getRutaFirma(contraseña);
                if (firmaUsuario) {
                  console.log('Firma obtenida:', firmaUsuario);
                  bot.sendMessage(chatId, 'Debe ingresar por mensajes separados los siguientes parámetros en el siguiente orden: ' + params[0], { reply_markup: JSON.stringify(replyMarkup) });
                  step = 2;
                } else {
                    await bot.sendMessage(chatId, 'Contraseña incorrecta o firma no encontrada.');
                }
              } catch (error) {
                  await bot.sendMessage(chatId, 'Hubo un error al obtener la firma. Por favor, inténtelo de nuevo.');
                  console.error(error);
                }
            }); */
          }
  ...
  if (msg.document.mime_type === 'text/csv') {
    ...
    parser.on('end', async () => {
      ...
      for (const obj of csvContent) {
        ...
        try {
          firmaUsuario = await dbConfig.getRutaFirma(contraseña)
          const documentData = await getDocumentData(documentName)//await dbConfig.getRutaFirma(contraseña)
          const pySpawner = new PythonSpawner(bot, msg.chat.id)
          //Pasar los mensajes al script de python para que cree un nuevo documento con ellos
      ...
      //console.log(values)
      //step = 3
      // Aquí puedes hacer lo que necesites con csvContent, como insertarlo en la base de datos
    });
    ...
    /*    const documentData = await getDocumentData(documentName)
       ...
       step = 1 */
  }/* else if (msg.document.mime_type === 'image/png' || msg.document.mime_type === 'image/jpg'){
      bot.sendMessage(chatId, 'Por favor ingrese una contraseña para poder acceder a la firma cuando se necesite')
      bot.once('message', async (msg)=>{
        contraseña = msg.text
        const docId = msg.document.file_id
        const filePathFIR = './../firmas/'
        const filePath2 = await bot.downloadFile(docId, filePathFIR)
        fs.renameSync(filePath2, filePath + msg.document.file_name)
        res = await dbConfig.newFirma(contraseña_nueva,filePathFIR + msg.document.file_name)
        console.log(res)
      });
  }   */
  else {
