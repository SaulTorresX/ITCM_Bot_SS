...
class Connection{
    ...
    async getRutaFirma(name){
        return new Promise((resolve, reject) =>{
            this.pool.query('SELECT ruta_firma FROM firmas WHERE contrasena = ?;', [name], (err, result, fields) => {
                if(err){
                    reject(err)
                }else{
    ...
    async newFirma(contrasena, ruta){
        return new Promise((resolve, reject) =>{
            this.pool.query('INSERT INTO firmas (contrasena, ruta_firma) VALUES (?, ?);',[contrasena, ruta] ,(err, result, fields) => {
                if(err){
                    reject(err)
                }else{
                    resolve(result)
